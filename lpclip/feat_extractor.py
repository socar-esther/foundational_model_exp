...
from datasets.sofar import SoFAR
...
ImageFile.LOAD_TRUNCATED_IMAGES = True # cropped sofar 이미지 처리
...
def main(args):
    ...
    args.model_dir = '../output/sofar/CLIP_Adapter/vit_b32_1500shots/nctx16_cscFalse_ctpend/seed1'
    args.load_model = None
    print(f'#### Load {args.model_dir}')
    trainer.load_model(args.model_dir) # 무조건 best로 간다
    clip_model = trainer.model
    clip_model.eval()
    ###################################################################################################################
    ...
    for train_step in range(1, len(train_dataiter) + 1):
        ...
        # clip adapter 포함한 형태로 feature extract하도록 진행
        feature_tmp = clip_model.image_encoder(data)
        feature = clip_model.adapter(feature_tmp)
        ratio = 0.8
        image_feature = ratio * feature + (1-ratio) * feature_tmp
        feature = image_feature / image_feature.norm(dim=-1, keepdim=True)
        feature = feature.cpu()
